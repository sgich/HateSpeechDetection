...
if query != '' and query != '#':
    with st.spinner(f'Searching for and analyzing {query}...'):
        ...
        tweets = tweepy.Cursor(tweetpy_creds.api.search,
                               q=new_search,
                               lang="en",
                               since=tweetpy_creds.date_since,  # until=date_until,
                               geocode=tweetpy_creds.geoc,
                               tweet_mode='extended').items(2)

        users_locs = [[tweet.full_text,
                       tweet.user.location,
        ...
        tweet_df = pd.DataFrame(data=users_locs, columns=['tweet', "location", 'friends_count', 'followers_count',
                                                          'tweet_date', 'retweet_count', 'hashtags'])
        ...
        for tweet in tweet_df['tweet']:
            ...
            if sentiment == "Hate Speech":
                ...
                tweet_data = tweet_data.append(
                    {'tweet': tweet, 'predicted-sentiment': sentiment, 'hate sub-cluster': clus_pp,
                ...
                non = ''
                tweet_data = tweet_data.append(
                    {'tweet': tweet, 'predicted-sentiment': sentiment, 'hate sub-cluster': non,
                     'confidence level': non}, ignore_index=True)
                tweet_data = tweet_data.reindex(
                    columns=['tweet', 'predicted-sentiment', 'hate sub-cluster', 'confidence level'])
